...
namespace AES {
    ...
    string getStrState(string text, unsigned int index) {
        string block = text.substr(index * 16, index * 16 + 16);
    ...
    bitset<8> mixColumnsMultiply(array<bitset<8>, 4> byte, unsigned short index) {
        ...
        for (unsigned short i = 0; i < 3; i++) {
            if (cBytes[i] == "01")
                intermidateRes[i] = byte[i];
            if (cBytes[i] == "02")
                intermidateRes[i] = (byte[i] << 1) ^ bitset<8>(0b0001'1011);
            if (cBytes[i] == "03")
                intermidateRes[i] = (byte[i] << 1) ^ bitset<8>(0b0001'1011) ^ byte[i];
        }
    ...
    array<bitset<8>, 4>& g(array<bitset<8>, 4> word, unsigned short round) {
        ...
        for (unsigned short RC = 0; RC < 3; RC++)
            RConArr[RC] = hexToBin(RCon[round + 1][RC]);
